...
def lateBooksQuery(date_start, date_end):
    ...
    sql_input_lbq = 'SELECT bl.book_id, bl.card_no, bl.Date_Out, bl.due_date, bl.Returned_Date, julianday(bl.Returned_Date) - julianday(bl.due_date) AS days_late FROM BOOK_LOANS AS bl WHERE bl.Returned_Date > bl.due_date AND bl.due_date >= ? AND bl.due_date <= ? '
